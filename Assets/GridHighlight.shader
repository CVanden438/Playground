   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         fixed4 frag (v2f i) : SV_Target
         {
            ...
            float2 centerOffset = i.uv;
            float distance = length(centerOffset);

            // Modify the alpha value based on the distance
            float fadeFactor = 1.0 - smoothstep(0.0, _FadeDistance, distance);
            ...
            if (pos.x == 1 || pos.y == 1) {
               col = _Color;
               col.a = max((1 - fade), fade) * fadeFactor;
            } else {
